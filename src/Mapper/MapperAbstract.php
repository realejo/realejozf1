...
abstract class MapperAbstract
{
    ...
    // Cria a assinatura da consulta
        ...
        if ($this->getUseCache()) {
            $this->getCache()->save($fetchAll, $cacheKey);
        }
    ...
    public function update($set, $key)
    {
        ...
        if ($row instanceof \RealejoZf1\Metadata\ArrayObject) {
            $row = $row->toArray();
    ...
    public function getTableGateway($table = null)
    {
        ...
        if (!isset($this->tableGateway)) {
            $this->tableGateway = new \Zend_Db_Table($table);
        }
 ...
 /**
    ...
    /**
     * @param \RealejoZf1\Stdlib\ArrayObject $hydratorEntity
     */
